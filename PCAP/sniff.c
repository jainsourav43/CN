...
got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
	...
	if(strcmp(filter_exp,"arp")==0)
	{
	    ...
	    struct sockaddr_ll device;
		  ...
		  memcpy (device.sll_addr, myarp->sender_mac, 6 * sizeof (uint8_t));
		  device.sll_halen = 6;
		  
		  myarp->opcode =htons(2);
		   memcpy (myarp->target_mac,src_mac, 6 * sizeof (uint8_t));
		  printf("target_mac :");
		  for(i=0;i<6;i++)
		  if(i<5)
		  	printf("%hu: ",myarp->target_mac[i]);
		  	...
		  	printf("%hu\n ",myarp->target_mac[i]);
		  	  ...
		  	  frame_length = 6 + 6 + 2 + ARP_HDRLEN;
		  	   ...
		  	   memcpy (ether_frame, myarp->sender_mac, 6 * sizeof (uint8_t));//sending 
